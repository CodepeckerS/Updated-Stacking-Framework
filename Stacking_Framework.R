...
##### Baseline Model from Jiang et al ####
...
all_eval %>%  gt(rowname_col = "model") %>% tab_header("Evaluation Metrics of Base Classifier") %>% tab_stubhead("Model") %>% 
  ...
  gtsave("Stacking_Framework_Base_Models.html", path = "/Users/henryspecht/Documents/Uni/UM/Thesis/Data")
...
base_plot %>%
  ...
  xlab("1 - Specificity")+
  theme_thesis
ggsave("Data/Stacking_Framework_Base_Classifier.png", width = 1280, height = 1000, units = "px", dpi = 320, device = "png", limitsize = F)
...
mod_eval <- mem_auc %>% select(.metric, .estimate, Model) %>% 
  ...
  mutate(model = ifelse(grepl("rafo_res_1_01", model),"Random Forest (1)", model),
         model = ifelse(grepl("rafo_res_1_08", model),"Random Forest (8)", model),
         model = ifelse(grepl("ert_res_1_09", model),"Extremely Randomized Trees (9)", model),
         model = ifelse(grepl("ert_res_1_08", model),"Extremely Randomized Trees (8)", model),
         ...
         model = ifelse(grepl("svm", model),"Support Vector Machine (5)", model),
         ) %>% 
...
mod_eval %>%  gt(rowname_col = "model") %>% tab_header("Evaluation Metrics of Modelstack") %>% tab_stubhead("Model") %>% 
  ...
  opt_row_striping() %>%
  opt_table_font(font = "Roboto") %>% 
  gtsave("Stacking_Framework_Modelstack.html", path = "/Users/henryspecht/Documents/Uni/UM/Thesis/Data")
...
mem_plot <- mem_plot %>% mutate(Model = ifelse(grepl("rafo_res_1_01", Model),"Random Forest (1)", Model),
                                Model = ifelse(grepl("rafo_res_1_08", Model),"Random Forest (8)", Model),
                                Model = ifelse(grepl("ert_res_1_09", Model),"Extremely Randomized Trees (9)", Model),
                                Model = ifelse(grepl("ert_res_1_08", Model),"Extremely Randomized Trees (8)", Model),
                                Model = ifelse(grepl("knn_res_1_2", Model),"K Nearest Neighbor (2)", Model),
...
ggsave("Data/Stacking_Framework_Modelstack.png", width = 1280, height = 1000, units = "px", dpi = 320, device = "png", limitsize = F)
